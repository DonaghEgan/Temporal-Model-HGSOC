...
ttest_two <- function(b1, b2, se_b1, se_b2, n){
  t = (b1 -b2)/sqrt((se_b1^2/n) + (se_b2^2/n))
  t = as.numeric(t)
  deg_free = n - 2
  # calculate t-test
...
meta_data_path <- "inputs/41588_2020_630_MOESM3_ESM.xlsx"
norm_counts_path <- "inputs/TreatmentNaive_log2exp_data.txt"
...
# Generate the plot
pdf("outputs/coefficients_krt_ifi.pdf", height = 2.5, width = 1.8)
...
# =====================================================
# Co-expression Analysis - IFI44L
# =====================================================

# Calculate correlations with IFI44L for all other genes
correlations <- data.frame(cor = sapply(norm_counts, function(x) cor(norm_counts$IFI44L, x))) %>%
  rownames_to_column("gene") %>%
  filter(gene != "IFI44L") %>%
  mutate(rank = rank(-cor, ties.method = "first"))

# Define TCR inhibition pathway and top 2% threshold
tcr_inhib <- pathways[["GOBP_NEGATIVE_REGULATION_OF_ALPHA_BETA_T_CELL_ACTIVATION"]]
top_2_percent_threshold <- ceiling(0.02 * nrow(correlations))

# Assign labels for plotting or further analysis
correlations <- correlations %>%
  mutate(
    label = ifelse(
      gene %in% tcr_inhib & rank < top_2_percent_threshold,
      gene,
      "NONE"
    )
  )

# Prepare named vector of correlations for fgsea analysis
genes <- correlations %>%
  arrange(cor) %>%
  dplyr::select(gene, cor) %>%
  deframe()
...
# Plot enrichment 
plotEnrichment(pathways[["GOBP_NEGATIVE_REGULATION_OF_ALPHA_BETA_T_CELL_ACTIVATION"]],
               genes) + labs(title = "GOBP_NEGATIVE_REGULATION_OF_ALPHA_BETA_T_CELL_ACTIVATION")
...
ggplot(correlations) +
  ...
  geom_vline(xintercept = top_5_percent_threshold, linetype = "dashed", color = "grey") +  # Add vertical line
  labs(x = "Rank", y = "Correlation") +  # Add axis labels and title
  theme_bw() + scale_color_manual(values = c("#F01015", "#F01015", "#F01015", "#000000"))
...
# Plot the results
pdf("/mnt/8TB/Projects/POIAZ/Donagh/protypia_proteomics/outputs/coefficients_cell_type.pdf", height = 2, width = 2.5)
